...
\begin{gitcommands}
  \item clean: To remove untracked files from the working directory.\\
  Options: -n, -f, -d, -x, -i\\
  Usage: git clean -f, git clean -n, git clean -i
  \vspace{8pt}
  \item filter-branch: A powerful and versatile tool used to rewrite the commit history of a Git repository.\\
  Options: --tree-filter, --commit-filter, --prune-empty\\
  Usage: git filter-branch --tree-filter 'rm -f [file]' -- --all
  \vspace{8pt}
  \item archive: Create a tar or zip archive of the contents of a Git repository or a specific Git tree (commit or branch). \\
  ...
  \vspace{8pt}
  \item bundle: Create a binary file containing the contents of a Git repository. \\
